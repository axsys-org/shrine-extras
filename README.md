...
Optional applications for Shrine. Each directory under `mounts/` is a flat source
bag. Core's `x/eden` selects this repository through its Nix pin or an editable
checkout:

```sh
../reaver-two/x/eden --extras-root . --mount explorer
../reaver-two/x/eden --extras-root . --mount debugger --mount srs --mount codex --check
```
...
The debugger's host entrypoint builds through this checkout's `flake.nix`, whose
Mash input is pinned. Source rescans refresh current checkout contents; changed
browser inputs rebuild their captured assets. Core's bare shell does not need
Node, pnpm, Mash or these application sources.

## Validation

Enter core's Nix shell (or set `WISP`) for Foil checks:

```sh
x/check --core-root ../reaver-two
x/check --core-root ../reaver-two --contracts-only
x/check --core-root ../reaver-two --mount life --mount codex
x/check-rescan --core-root ../reaver-two
SHRINE_CORE_ROOT=../reaver-two x/hyp --check
SHRINE_CORE_ROOT=../reaver-two python3 mounts/codex/codex_bridge_tests.py
SHRINE_CORE_ROOT=../reaver-two nix develop .#debugger --command python3 mounts/debugger/check-debugger.py
```
