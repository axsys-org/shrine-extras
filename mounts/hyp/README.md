...
```sh
SHRINE_CORE_ROOT=../reaver-two WISP=/path/to/working/wisp x/hyp --check
WISP=/path/to/working/wisp x/check --core-root ../reaver-two --mount hyp
```

`x/hyp` compiles the real Grove schema, publishes it under `/<host>/gov/hyp`,
