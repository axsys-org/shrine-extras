...
def check(args):
    ...
    fixture = fixture_repository()
    print('Debugger check artifacts: ' + str(work), flush=True)
    code, complete, _ = runner.run_process([sys.executable,str(BAG / 'debug-runtime-tests.py')],
    ...
    template = Path(os.environ['TEST_TEMPLATE']) if os.environ.get('TEST_TEMPLATE') else runner.stage(wisp, False, 1800)
    runner.copy_template(template, work / 'native/snap')
    before = source_digest(fixture)
    source = '\n'.join([
    ...
    code, complete, seconds = runner.run_process(
        [wisp, '--file-root', str(fixture / 'src'), 'snap', 'root', '_'],
        work / 'native', source, 1800)
    ...
    if source_digest(fixture) != before:
        raise RuntimeError('Captured fixture sources changed during compilation; rerun the gate.')
    ...
    if not args.backend_only:
        ...
        for name, command in [('build-boundary',[node,str(BAG / 'debug-build-test.mjs')]),
                              ('read-queue',[node,str(BAG / 'debug-preview-queue-test.mjs')]),
                              ('values',[node,str(BAG / 'debug-values-test.mjs')]),
                              ('physical-parser',[node,str(BAG / 'debug-physical-contract-test.mjs')]),
                              ('assets',[node,str(BAG / 'build-debug.mjs'), *(['--release'] if args.release else [])]),
                              ('browser',[node,str(BAG / 'debug-declaration-test.mjs')])]:
