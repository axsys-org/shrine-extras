...
```sh
x/eden --extras-root ../shrine-extras --mount codex
x/eden --extras-root ../shrine-extras --mount codex --mount srs --check
```
...
Run `python3 mounts/codex/bridge.py` from the workspace Codex should
...
```sh
WISP=/path/to/wisp x/check --core-root ../reaver-two --mount codex
python3 mounts/codex/codex_bridge_tests.py
```
...
Run `node mounts/codex/codex_ui_browser_tests.cjs` to check the
